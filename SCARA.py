...
#Inner functions
    def _connect(self):
        ...
        for attempt in range(5):
            ...
            try:
                self.connection.read_register(self.REG.STATE,functioncode=3)
                print(f"  Succes")
    ...
    def _enum_state (self, numb):
        states =  ["ErrorStop", "Disabled", "Standstill", "Stopping", "Motion","Homing"]
        return states[numb]
...
#General functions
    ...
    def Power (self, Enable: bool) -> bool:
        self.Power.Status = True
        """Enables power the robot motors"""
        try:
            self.connection.write_bit(self.REG.POWER,bool)
            return 1
        except IOError as error:
            print (f"Failed to write data to robot: {type(error).__name__} - {error}")
            return 0

    def Show_Status(self, Enable: bool) -> bool:
        """Show the status of the robot in the terminal"""
        """Enable = True: Show status in terminal
           Enable = False: Hide status in terminal"""
        if Enable == True:
            stdscr = curses.initscr()
            curses.curs_set(0)  # Hide cursor
            stdscr.clear()

            stdscr.addstr(0,  0, "SCARA robot")
            stdscr.addstr(1,  0,  F"_________________________________________________________________________________________________")
            stdscr.addstr(2,  0, f"port     |  {self.port}")
            stdscr.addstr(3,  0, f"address  |  {self.address}")
            # stdscr.addstr(3,  0, f"baudrate |  {self.connection.serial.baudrate}")

            stdscr.addstr(4,  0,  F"_________________________________________________________________________________________________")
            stdscr.addstr(5,  0,  F"Motor ON       | {self.Power}")
            stdscr.addstr(5,  50, F"Moving         | {self.Moving}")
            # stdscr.addstr(7,  0,  F"On target      | {self.target_reached()}")
            stdscr.addstr(7,  50, F"Homed          | {self.homed}")
            
            
            stdscr.addstr(9,  0, F"_________________________________________________________________________________________________")
            stdscr.addstr(10, 0, F"Axis          |")
            stdscr.addstr(10, 20, F"A")
            stdscr.addstr(10, 40, F"B")
            stdscr.addstr(10, 60, F"C")
            stdscr.addstr(10, 80, F"Z")
            
            stdscr.addstr(11, 0, F"speeds        |")
            stdscr.addstr(11, 20, F"{self.speeds[0]}")
            stdscr.addstr(11, 40, F"{self.speeds[1]}")
            stdscr.addstr(11, 60, F"{self.speeds[2]}")
            stdscr.addstr(11, 80, F"{self.speeds[3]}")

            stdscr.addstr(12, 0, F"Acceleration  |")
            stdscr.addstr(12, 20, F"{self.accelerations[0]}")
            stdscr.addstr(12, 40, F"{self.accelerations[1]}")
            stdscr.addstr(12, 60, F"{self.accelerations[2]}")
            stdscr.addstr(12, 80, F"{self.accelerations[3]}")

            stdscr.addstr(13, 0, F"Target        |")
            stdscr.addstr(13, 20, F"{self.targets[0]}")
            stdscr.addstr(13, 40, F"{self.targets[1]}")
            stdscr.addstr(13, 60, F"{self.targets[2]}")
            stdscr.addstr(13, 80, F"{self.targets[3]}")

            stdscr.addstr(14, 0, F"Position      |")
            stdscr.addstr(14, 20, F"{self.positions[0]}")
            stdscr.addstr(14, 40, F"{self.positions[1]}")
            stdscr.addstr(14, 60, F"{self.positions[2]}")
            stdscr.addstr(14, 80, F"{self.positions[3]}")

            stdscr.addstr(15, 0, F"Motor Pos     |")
            stdscr.addstr(15, 20, F"{self.motor_positions[0]}")
            stdscr.addstr(15, 40, F"{self.motor_positions[1]}")
            stdscr.addstr(15, 60, F"{self.motor_positions[2]}")
            stdscr.addstr(15, 80, F"{self.motor_positions[3]}")

            stdscr.addstr(16,  0, F"_________________________________________________________________________________________________")
            stdscr.refresh()
        else:
            curses.endwin()

    def Read_Error(self) -> str:
        """Read error code from robot"""
        try:
            error_Code = self.connection.read_register(self.REG.ERROR_CODE,functioncode=4)
        except IOError as error:
            print (f"Failed to read error code from robot: {type(error).__name__} - {error}")
            return
        bin_string = format(error_Code, '16b')
        Error_Bools = [x == '1' for x in bin_string[::-1]]

        return Error_Bools


#Movement commands
    def Home (self, Execute,Position) -> bool:
        return
...
#read variables functions
    def read_targets(self) -> list[int]:
        ...
        targets = self._read(self.REG.A_TARGET,4)
    ...
    def read_speeds(self) -> list[int]:
        ...
        speeds = self._read(self.REG.A_VEL,4)
        try:
    ...
    def read_accelerations(self)-> list[int]:
        ...
        accelerations = self._read(self.REG.A_ACC,4)
        try:
    ...
    def read_positions(self)-> list[int]:
        ...
        try:
            self.positions = self.connection.read_registers(self.REG.A_POS,4,4)     
        except IOError as error:
            print (f"Failed to read data from robot: {type(error).__name__} - {error}")
            return
    ...
    def read_motor_positions(self)-> list[int]:
        """Return current robot position"""
        try:
            self.motor_positions = self.connection.read_registers(self.REG.M_A_POS,4,4)     
        except IOError as error:
            print (f"Failed to read data from robot: {type(error).__name__} - {error}")
            return
...
if __name__ == "__main__":
    ...
    robot = SCARA("COM5")
    # robot.Power(Enable = True)
    # robot.port = 5
    speed = 100
    accel = 1000

    while True:
        robot.Show_Status(Enable = True)
